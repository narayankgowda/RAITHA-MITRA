...
import { mockAdminData, AdminProfileData } from '../data/userData';
import { UserIcon, ShieldCheckIcon } from './icons';

const AdminProfile: React.FC = () => {
    const [isEditing, setIsEditing] = useState(false);
    const [profileData, setProfileData] = useState<AdminProfileData>(mockAdminData);
    const [tempData, setTempData] = useState<AdminProfileData>(mockAdminData);
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <div className="bg-card-light dark:bg-card-dark shadow-lg rounded-lg p-6 border border-border-light dark:border-border-dark">
                <div className="flex flex-col md:flex-row items-center md:items-start md:space-x-6">
                    <div className="flex-shrink-0 mb-4 md:mb-0">
                        <img className="h-32 w-32 rounded-full object-cover border-4 border-slate-500" src={profileData.profilePicture} alt="Admin Profile" />
                    </div>
                    <div className="flex-grow text-center md:text-left">
                     ...
                     <div className="mt-4 md:mt-0">
                        {!isEditing ? (
                            <button onClick={() => setIsEditing(true)} className="px-4 py-2 bg-slate-600 text-white font-semibold rounded-lg hover:bg-slate-700">Edit Profile</button>
                        ) : (
                ...
                <div className="mt-8 border-t border-border-light dark:border-border-dark pt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <DetailItem label="Email Address" value={profileData.email} isEditing={isEditing}>
                     ...
                     <DetailItem label="Role" value={profileData.role} isEditing={isEditing}>
                         <input type="text" name="role" value={tempData.role} onChange={handleInputChange} className="mt-1 block w-full input-style" />
                    </DetailItem>
                     <div className="md:col-span-2">
                        <label className="block text-sm font-medium text-gray-500 dark:text-gray-400">Permissions</label>
                         <div className="mt-2 flex flex-wrap gap-2">
                            {profileData.permissions.map((permission, index) => (
                                <span key={index} className="px-3 py-1 text-sm font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/50 dark:text-blue-300 rounded-full flex items-center">
            ...
            <style>{`.input-style {padding: 0.5rem 0.75rem; background-color: white; border: 1px solid #e2e8f0; border-radius: 0.375rem;} .dark .input-style {background-color: #334155; border-color: #475569;} .input-style:focus {outline: 2px solid transparent; outline-offset: 2px; --tw-ring-color: #64748b; border-color: #64748b;}`}</style>
        </div>
    );
};
